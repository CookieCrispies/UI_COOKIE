...
const ExtendModal: React.FC<DepositModalProps> = ({ amount, onConfirm, onDismiss, tokenName = '', addLiquidityUrl, title, existingStakeDuration , existingStakeSecondsRemain}) => {
    ...
    if (title === 'Convert to Lock' || title === 'Renew') {
        warningMessage = 'You will be able to withdraw the staked Cookie and profit only when the staking position is unlocked.';
    }
    ...
    return (
        <Modal title={title} onDismiss={onDismiss}>
            <Text>Amount: {Number(amount).toFixed(3)} Cookie</Text>
            ...
            <div style={{marginBottom: "20px"}} />
            <Text><strong>APR:</strong> {lockedApy ? new BigNumber(getLockedApy(secondsDuration)).toFixed(2) : "0.00"}%</Text>
            <Text><strong>You will earn:</strong> {youWillEarn && !Number.isNaN(Number(youWillEarn)) ? youWillEarn : "0.000"} Cookie</Text>
